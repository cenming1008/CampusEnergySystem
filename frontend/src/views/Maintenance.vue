...
const dialogTitle = computed(() => dialogType.value === 'create' ? '新建维护计划' : '完成维护')
...
const handleSubmit = async () => {
  if (dialogType.value === 'create') {
    if (!formData.device_id || !formData.title || !formData.scheduled_time) {
...
<template>
  <div class="maintenance-page">
    ...
    <div class="stat-cards">
      <div class="stat-card">
        <div class="stat-icon scheduled"><el-icon><Calendar /></el-icon></div>
        <div class="stat-info">
          <div class="stat-value">{{ statistics.by_status?.scheduled || 0 }}</div>
          <div class="stat-label">计划中</div>
        </div>
      </div>
      <div class="stat-card">
        <div class="stat-icon in-progress"><el-icon><Loading /></el-icon></div>
        <div class="stat-info">
      ...
      <div class="stat-card warning">
        <div class="stat-icon overdue"><el-icon><Warning /></el-icon></div>
        <div class="stat-info">
    ...
    <div class="toolbar">
      <div class="filters">
        <el-select v-model="filters.device_id" placeholder="选择设备" clearable style="width: 180px">
          <el-option v-for="d in deviceList" :key="d.id" :label="d.name" :value="d.id" />
        </el-select>
        <el-select v-model="filters.maintenance_type" placeholder="维护类型" clearable style="width: 140px">
    ...
    <el-tabs v-model="activeTab" class="maintenance-tabs">
        ...
        <el-table :data="maintenanceList" v-loading="loading" stripe>
          ...
          <el-table-column label="类型" width="100">
            <template #default="{ row }">
              <el-tag size="small">{{ typeLabel(row.maintenance_type) }}</el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column label="计划时间" width="160">
            <template #default="{ row }">{{ formatDateTime(row.scheduled_time) }}</template>
          </el-table-column>
      ...
      <el-tab-pane :label="`即将到期 (${upcomingList.length})`" name="upcoming">
        <el-table :data="upcomingList" stripe>
          <el-table-column prop="id" label="ID" width="60" />
          <el-table-column label="设备" width="150">
            <template #default="{ row }">{{ deviceName(row.device_id) }}</template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="120" fixed="right">
            <template #default="{ row }">
              <el-button type="primary" link size="small" @click="handleStart(row)">开始</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>

      <el-tab-pane :label="`逾期未完成 (${overdueList.length})`" name="overdue">
        <el-alert v-if="overdueList.length > 0" type="warning" :closable="false" show-icon>
          有 {{ overdueList.length }} 条维护计划已逾期，请尽快处理！
        </el-alert>
        <el-table :data="overdueList" stripe style="margin-top: 15px">
          <el-table-column prop="id" label="ID" width="60" />
          <el-table-column label="设备" width="150">
            <template #default="{ row }">{{ deviceName(row.device_id) }}</template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="150" fixed="right">
            <template #default="{ row }">
              <el-button type="primary" link size="small" @click="handleStart(row)">立即处理</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <!-- 新建表单 -->
      <el-form v-if="dialogType === 'create'" label-width="100px">
        <el-form-item label="设备" required>
          <el-select v-model="formData.device_id" placeholder="选择设备" style="width: 100%">
            <el-option v-for="d in deviceList" :key="d.id" :label="d.name" :value="d.id" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="维护标题" required>
        ...
        <el-form-item label="维护人员">
          <el-input v-model="formData.operator" placeholder="请输入维护人员" />
        </el-form-item>
      ...
      <el-form v-else label-width="100px">
        <el-form-item label="维护结果">
          <el-input v-model="completeFormData.result" type="textarea" :rows="3" placeholder="请输入维护结果" />
        </el-form-item>
        <el-form-item label="维护成本">
          <el-input-number v-model="completeFormData.cost" :min="0" :precision="2" style="width: 100%" />
        </el-form-item>
        <el-form-item label="更换部件">
          <el-input v-model="completeFormData.parts_replaced" placeholder="请输入更换的部件" />
        </el-form-item>
