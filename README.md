...
- 📋 [工业上线清单](./docs/03-开发与部署/工业上线清单.md) - 上线前检查与验收
...
- [x] **后端自动化测试**：`tests/` 已建立并接入 GitHub Actions
- [x] **API 限流**：已覆盖登录、设备控制、设备上报、导出、前端异常上报及全局限流
- [x] **数据库迁移**：已提供 Alembic 基线与后续迁移
- [x] **可观测性**：已提供 `/metrics`、Prometheus、Grafana、Alertmanager 基线
- [x] **CI/CD 基线**：已包含后端 CI、前端 CI、前端 E2E、远端部署 workflow
- [x] **安全护栏**：已提供 Trusted Hosts、HTTPS 重定向、安全头、监控端点访问控制、首次登录强制改密

### 📋 当前主要短板

#### 正式投产前建议补齐
- [ ] 正式企业告警通道接入与真实送达验收
- [ ] 远端部署 workflow 在真实环境完成实跑
- [ ] 现场真实设备联调并冻结接入协议
- [ ] 真实备份、恢复、回滚演练留痕
- [ ] 长时间稳定性观察（7-14 天）
- [ ] 高可用部署方案（后端多实例 / 数据库与缓存高可用）
