...
- `campus_service.py` 仍是 `split_candidate`，但已处理 energy category summary、subitem statistics、realtime load trend 与 location rankings 四个纯聚合 helper；后续继续整理时仍必须一次只选一个独立 helper。
