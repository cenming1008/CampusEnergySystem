...
- 容量、额定功率、SOC 上下限、充放电效率和爬坡率均通过 `StorageAssetConfig` 显式配置，不得在领域模型中硬编码；`500 kWh / 250 kW` 与 `10%-90%` 只是本阶段默认验收场景 / 基准配置。SOC 使用 `0-100` 百分比，功率用 `kW`、能量用 `kWh`、时间输入用秒。
