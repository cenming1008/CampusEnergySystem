...
- 2026-04-23：已补做控制台前端单测收敛与文档同步；`DeviceControlConsole` 相关测试桩件已与当前组件分层对齐，避免旧测试继续误报控制台视图异常。
- 2026-04-23：已完成 `P1` 第一轮关键监控指标真实化压缩：`CapacitorBankControlProfile` 现已真正承载 `running_circuit_count / control_mode / auto_*` 等快照字段，电容补偿控制器监控页在无遥测但有参数快照时会优先使用 `profile` 来源，不再直接退回 `configured_fallback / estimated`。
- 2026-04-23：已继续完成 `P1` 温度健康度收口：电容补偿控制器监控页已新增 `temperature_health` 语义，前端概览卡片会优先按实时温度告警位和参数上限回读显示“正常 / 接近上限 / 超过上限 / 温度告警 / 待判断”。
- 2026-04-24：已完成补偿器控制链路 UAT 打磨：控制回执主动推送、控制链路结构化日志、JKWF 解码异常 warning、控制回执超时配置化、SQLite 写参风险启动提示与前端控制日志事件刷新均已落地；该轮不改变真实设备协议未冻结的正式完善门槛。
- 2026-04-24：已完成补偿器真实网关适配系统侧收敛：能力接口新增 `remote_commands / writable_parameters`，默认禁用真实网关未确认的 `reset_alarm`，参数写入收窄到 6 个低风险字段，控制回执兼容真实网关拒绝别名并保护终态不被迟到回执覆盖；本轮不修改外部工控网关/采集脚本。
- 2026-05-14：规则角色补充告警职责边界：设备运行类告警优先来自补偿控制器上报，平台负责解析、标准化、归档和生命周期，平台通讯类告警可由平台根据心跳或数据更新时间补充生成；监控页面只展示告警与事件，不承担核心告警判定。
- 2026-05-14：已按告警边界完成实现收敛：`Alarm.source` 区分 `device_native/platform_rule/platform_comm`；补偿设备原生状态位进入 `device_native`，参数门限与过补偿推导进入 `platform_rule`，补偿设备不再默认套用通用电流 / 电压阈值告警；接入健康会创建 / 恢复 `platform_comm/communication_offline` 通讯告警；告警中心与补偿监控告警表展示来源标签。
- 2026-05-14：已继续补齐剩余闭环：默认 scheduler 每分钟执行 `sync_platform_comm_alarms` 扫描全量接入健康记录，旧 `source=telemetry` 告警在前端显示为“历史遥测”，本轮不做历史数据迁移。
...
1. 先完成真实设备/真实网关回执协议核对，冻结 `topic / payload / command_id` 关联口径。
2. 再完成关键监控指标真实化，优先替换控制模式、容量利用率、柜内温度健康度中的估算/占位来源。
3. 再决定高风险动作边界：继续关闭，或联调验证后开放。
4. 最后由验收角色按真实链路证据判断是否从 `MVP+` 升级为“正式完善 / 可正式收口”。
