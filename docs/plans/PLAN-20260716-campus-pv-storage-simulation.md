...
- 本 PLAN、详细实施计划、状态与交接统一以 `docs/superpowers/specs/2026-07-17-single-storage-system-convergence-design.md` 为收敛依据。
- 项目只保留一个储能设备模型、一套持久化/服务/API 和原有 `StorageMonitorView`；模拟器与未来厂商网关只是实现同一 MQTT 契约的可替换数据源。
- 园区级光储能力进入现有 `EnergyManagement` 页面中的“光储 EMS”工作区，不新增 `/storage-energy` 独立路由。
...
- 储能 migration 链固定为 `20260716_0001 -> 20260716_0002 -> 20260717_0003`；新增 revision 必须继续静态、offline-safe。
...
- Task 4 至 Task 15 已完成。开发库已显式升级，MQTT、只读 cutover preview、真实遥测/控制回执与原页面人工验收均已通过；未执行 cutover `--execute`。
...
- 不建立模拟版与真实版两套储能业务系统，不新建模拟设备页面，不让业务后端直接耦合厂商私有协议。
...
- 真实厂商网关必须使用相同 MQTT 契约并标记 `data_source=real`；平台不得根据设备编号猜测来源。
- EMS 自动控制全局和单设备均默认关闭，单设备完成能力、安全和回执验收后才允许管理员启用。
...
- 实现提交：`0e1fd1bc`。
- MQTT 扩展接入已覆盖目标功率、可充放电功率、命令来源与数据来源，并保持负功率表示放电。
- 设备侧 `bms_state`、`pcs_state`、`grid_connection_state` 已显式映射为持久化列 `bms_status`、`pcs_status`、`grid_status`。
- 监控聚合已输出目标/实际功率、可充放电能力、BMS/PCS/并网状态、命令来源和模拟数据来源语义。
- TDD RED 由未提取的新字段和缺失的 monitor 指标触发；focused 与治理状态测试恢复 GREEN。
- 完整环境全量后端验证为 `742 passed, 5 warnings`；Task 4 变更文件 Ruff 检查通过。

## Task 5 实现与验收证据

- 新增可直接执行的 `scripts/python/storage_simulator.py`，覆盖五个固定场景、固定 seed、加速时钟、储能状态演化与优雅停止。
- `--print-only` 不创建 MQTT 客户端；正常模式发布 `campus/device/{device_code}/telemetry`，并分别监听真实控制和 simulator-only 控制主题。
- 模拟 payload 固定分类、子型和正充负放约定，并强制携带 `data_source=simulated`；控制回执同样明确标记模拟来源。
- 新增 `storage_ems_enabled`、`storage_simulation_enabled`、仿真主题前缀和每日调度时间设置，两个 enable 门禁在 Python 与所有环境模板中均默认关闭。
- TDD RED 由缺失模块触发；直接脚本入口的导入失败另由子进程回归测试复现并按仓库入口模式修复。
- 聚焦测试 `12 passed`，完整环境全量后端验证为 `754 passed, 7 warnings`；Task 5 变更文件 Ruff 与差异检查通过。
...
4. Task 4，后端储能角色：扩展仿真遥测接入与状态字段映射已完成并通过验收。
5. Task 5，后端储能角色：可复用 MQTT 储能仿真器 CLI 与默认关闭运行门禁已完成并通过验收。
6. Task 6，后端储能角色：实现储能命令生命周期、分类回执分发与超时收敛。
7. Task 7-9，后端储能角色：扩展原有储能 API、设备级自动控制门禁、规则 EMS 与模拟器执行回执。
8. Task 10，前端角色：只增强原有 `StorageMonitorView`，不创建模拟版页面。
9. Task 11-13，后端储能角色：完成日前优化、计划执行和同一领域服务下的园区级聚合。
10. Task 14，前端角色：在现有 `EnergyManagement` 页面增加“光储 EMS”工作区，不新增独立路由；已完成。
11. Task 15，验收/后端角色：完成端到端演示、按设备模拟数据清理预览和适配器替换验收。
12. 每个阶段结束后由验收角色核对固定契约、测试证据、非目标和剩余风险。
...
- `20260717_0003` 已固定接在 `20260716_0002` 后；后续 migration 必须继续保持 offline-safe，不能重新引入动态 metadata 或在线检查。
- Task 3/7 最初使用固定临时数据库验收；现场验收已将实际开发库显式升级到 `20260717_0003` 并复核表、列、索引与 hypertable。未来 migration 仍不得把临时路径通过等同于业务库已升级。
- 模拟转真实时只能按精确设备和 `data_source=simulated` 预览、复核并事务清理业务记录；禁止全表清空，设备档案、权限、真实记录和清理审计必须保留。
...
- Task 4：扩展仿真遥测入库与监控聚合通过，正式完成。
- Task 5：可复用 MQTT 储能仿真器与默认关闭门禁通过，正式完成。
- Task 6：储能控制命令生命周期、分类回执分发与超时收敛通过，正式完成。
- Task 7：来源/运行标识、设备级自动门禁及原有嵌套储能 API 通过，正式完成。
- Task 8：安全优先纯规则、双门禁 EMS 编排与默认关闭任务通过，正式完成。
- Task 9：模拟器命令执行、稳定运行标识、终态幂等和独立故障注入通过，正式完成。
- Task 10：原有储能设备工作台、权限门禁、命令时间线和目标/实际功率展示通过，正式完成。
- Task 11：确定性 96 时段日前 MILP 优化器、开源 CBC 依赖与输入/不可行校验通过，正式完成。
- Task 12：调度计划原子替换、实时 EMS 安全边界、失败回退、每日任务和嵌套 API 通过，正式完成。
- Task 13：园区级光储总览、同输入三策略重放、来源/时效/计划状态和显式响应契约通过，正式完成。
- Task 14：原有能耗分析页光储 EMS 工作区、权限门禁、异步状态保护和来源/求解状态展示通过，正式完成。
- Task 15：正式完成。开发库从 `20260716_0001` 升级到 `20260717_0003`，两张储能表、扩展列、索引和 `energydata` hypertable 均保留；升级前备份已验证可读。
- 验收阶段 A：真实 MQTT 入库完成，STO-001 仿真遥测带稳定 `simulation_run_id` 与 `data_source=simulated`；阶段 B：两条控制命令均经历真实设备回执并终态成功；阶段 C：确定性三策略比较与原 `/energy` 工作区展示通过。
- 现场只读 cutover preview 为 `telemetry=259, plans=0, control_logs=2`；从未执行 `--execute`。自动控制保持全局与设备级默认关闭。
- 原 `StorageMonitorView` 与 `/energy` 光储 EMS 工作区人工验收通过；模拟来源、目标/实际功率、回执生命周期、`plan_execution_rate=null -> --` 与笔记本布局符合契约。
- 下一角色：未来真实厂商网关接入角色；仅替换设备侧适配器并标记 `data_source=real`，不得新建第二套储能系统。
