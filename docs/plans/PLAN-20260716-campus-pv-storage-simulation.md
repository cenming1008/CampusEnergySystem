...
- Task 4 至 Task 14 已完成；Task 15 的代码与离线验收已完成，现场验收仍需先显式升级开发库、恢复 MQTT 健康，再执行只读 preview、真实 MQTT 和人工页面验证。
...
- Task 3/7 验收使用固定临时数据库；实际开发库尚未随本任务升级。启动依赖新表或新列的运行链路前，必须显式升级到 `20260717_0003` 并复核表、列和 hypertable，不得把临时路径通过等同于开发库已升级。
...
- Task 15：代码、精确设备 cutover 测试、确定性演示、适配器替换测试与离线验收通过；现场验收待执行，尚未正式收口。
- 验收阶段 A：代码与离线验收完成；真实 MQTT 集成验收等待开发库显式升级。阶段 B、C 尚未开始。
- 下一角色：现场验收角色；先获得开发库升级授权并恢复 MQTT 健康，再完成 Task 15 Step 9-12，禁止执行 cutover `--execute`。
