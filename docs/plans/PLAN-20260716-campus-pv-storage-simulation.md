...
- Task 4 至 Task 13 已完成；单一储能持久化、仿真、控制闭环、设备工作台、日前优化、计划安全执行和园区级聚合均已就绪，下一角色执行 Task 14 原有能耗分析页“光储 EMS”工作区。
...
10. Task 14，前端角色：在现有 `EnergyManagement` 页面增加“光储 EMS”工作区，不新增独立路由。
...
- 下一角色：前端角色，按 TDD 执行 Task 14，在现有 `EnergyManagement` 页面增加“光储 EMS”工作区，不新增路由。
