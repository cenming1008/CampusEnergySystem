...
- SOC 单位为百分比，取值范围为 `0-100`；本任务硬边界为 `10%-90%`。
- 功率单位为 `kW`，正功率充电、负功率放电。
- 能量单位为 `kWh`，时间输入单位为秒。
- 额定容量固定为 `500 kWh`，额定充放电功率固定为 `250 kW`。
- 充电效率 `ηc`、放电效率 `ηd` 和爬坡率为显式配置；效率必须在 `(0, 1]`，爬坡率必须为有限正数。
...
- 每一步先应用爬坡约束 `|P_t - P_(t-1)| <= ramp_rate_kw_per_s * seconds`，再应用额定功率约束 `|P_t| <= 250 kW`，最后按 `10%-90%` SOC 硬边界对实际功率和能量变化进行饱和。
...
1. Task 1，规则角色：提交主题迁移、固定契约和持久化门禁记录，完成质量整改并等待复核结论。
...
- Task 1：治理实现已提交，当前处于质量整改 / 复核阶段，尚未取得验收通过结论。
