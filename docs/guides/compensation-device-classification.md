...
- 电容补偿控制器的过压、欠压、谐波超限、温度告警、投切异常、电容器或回路异常等设备运行类告警，应优先来自控制器上报的告警码、故障码、状态字或事件记录。
- 平台负责把设备原生告警标准化为业务告警并维护生命周期，不应要求监控页面根据电压、电流、功率因数、无功功率或投切状态自行生成核心告警。
- 通信中断、数据长时间未更新、平台接收异常等平台通讯类告警，可以由平台基于心跳或最后数据时间补充生成，并应和设备原生告警区分来源。
- 电容补偿控制器状态位、故障码或告警码触发的告警来源统一标记为 `device_native`。
- 参数门限、平台推导的过补偿等补充规则来源统一标记为 `platform_rule`。
- 补偿设备不再默认套用通用 `current_overload` / `voltage_out_of_range` 阈值告警；如需启用，必须作为补偿类专属平台规则单独声明。
