...
2026-05-16 现场真实设备数据已进入平台。本轮联调使用以下系统侧入口：

| 用途 | 地址 / 端口 | 说明 |
| --- | --- | --- |
| MQTT 遥测入站（dev 明文） | `192.168.1.46:1883` | 现场网关 / 真实设备发布遥测数据到该地址。 |
| MQTT 遥测入站（TLS） | `192.168.1.46:8883` | 启用 TLS 时使用，客户端需配置平台 CA。 |
| 后端 API / 健康检查 | `http://192.168.1.46:8088` | 后端服务当前监听 `0.0.0.0:8088`，可用于局域网内联调检查。 |

当前 `.env` 中 `MQTT_BROKER=localhost` 是平台 worker 连接本机 broker 的内部配置；设备侧网关应使用上表的局域网地址连接系统侧 MQTT broker。
