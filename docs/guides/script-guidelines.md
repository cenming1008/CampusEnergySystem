...
以下对象如果出现在根目录，默认按“运行产物 / 本地噪音”处理，而不是主入口：
...
- 这类对象优先通过 `.gitignore`、主区文档和规范说明治理
- “治理”不等于当前轮立即执行删除、迁移或清空
- 若涉及真实清理、目录迁移、对象去留判断，必须另行锁边界
- 不得借“噪音治理”顺手处理 `five-thread-orchestrator/` 或扩到仓库结构重构
- 发布、验收、回滚、备份、恢复工具
- 模拟器、网关、压测和验证工具
